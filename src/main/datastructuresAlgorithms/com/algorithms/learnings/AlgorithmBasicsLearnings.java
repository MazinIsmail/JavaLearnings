...
/**
 * Algorithm Analysis Efficiency of an algorithm can be analyzed at two
 * different stages, before implementation and after implementation. They are
 * the following:-
 * 
 ...
 * 
 * 
 * Algorithm Complexity Suppose X is an algorithm and n is the size of input
 * data, the time and space used by the algorithm X are the two main factors,
 ...
 * memory space required by the algorithm in its life cycle. The space required
 ...
 * 
 * 2. A variable part is a space required by variables, whose size depends on
 * the size of the problem. For example, dynamic memory allocation, recursion
 * stack space, etc.
 * 
 ...
 * 
 * 
 * Time Complexity: Time complexity of an algorithm represents the amount of
 * time required by the algorithm to run to completion. Time requirements can be
 * defined as a numerical function T(n), where T(n) can be measured as the
 ...
 * addition of two bits. Here, we observe that T(n) grows linearly as the input
 * size increases.
 */
public class AlgorithmBasicsLearnings {
